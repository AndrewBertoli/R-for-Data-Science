...
> side note - a good practice problem here is to use break to find the first number that is divisible by 3 ,4, and 7 -- for(i in 1:100){if(i%%3==0&i%%4==0&i%%7==0)break} 
...
> side note - could be good to discuss how lapply can be used to merge datasets by writing a function that takes information from one dataset (a row at a time), identifies the corresponding row in the other dataset, and then extracts the desired information  
...
> side note - teaching students to make their own packages may be a little advanced for an intro course. It might make more sense to cover how to modify functions from exisitng packages to adjust what they do
